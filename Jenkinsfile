...
pipeline {
    ...
    stages {
        stage('Checkout Git') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[credentialsId: 'Github-credential', url: 'https://github.com/special-PPT/cloud-native-microservice-strangler-example.git']]])
            }
        }
        stage('Build') {
            steps {
